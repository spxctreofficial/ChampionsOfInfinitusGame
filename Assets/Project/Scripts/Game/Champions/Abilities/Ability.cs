...
public class Ability : MonoBehaviour
{
	...
	private IEnumerator QuickAssist()
	{
		...
		foreach (ChampionController selectedChampion in GameManager.instance.champions)
		{
			...
			Debug.Log(abilityScriptableObject.abilityName + " was activated for " + championController.champion.championName + ". Dealing that champion a card!");
	...
	private IEnumerator Rejuvenation()
	{
		...
		Debug.Log(abilityScriptableObject.abilityName + " was activated for " + championController.champion.championName + ". Healing for 5.");
	...
	private IEnumerator Smite(int amount)
	{
		...
		yield return StartCoroutine(markedForSmite.Damage(amount, DamageType.Lightning, championController, true));
