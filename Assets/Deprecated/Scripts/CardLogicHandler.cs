...
public class CardLogicHandler : MonoBehaviour
{
        ...
        if (cardOfPlayer)
        {
            switch (gameHandler.phase)
            {
                case GamePhaseDeprecated.PLAYERACTIONPHASE:
                    ...
                    break;
                case GamePhaseDeprecated.PLAYERENDPHASE:
            ...
            if (selectedCardComponent.cardSuit == CardSuit.DIAMOND && gameHandler.opponent.diamondsBeforeExhaustion != 0)
            {
                switch (selectedCardComponent.cardValue)
                {
                    ...
                    case 4:
                        if (gameHandler.player.cards == 0)
						{
                            gameHandler.player.Damage(20, DamageTypeDeprecated.Unblockable, gameHandler.player);
                    ...
                    case 10:
                        ...
                        yield break;
                    case 11:
                        gameHandler.player.Damage(20, DamageTypeDeprecated.Fire, gameHandler.opponent);
                        Discard(selectedCard, true);
                        gameHandler.opponent.diamondsBeforeExhaustion--;
                        StartCoroutine(OpponentCardLogic());
                        yield break;
                    case 12:
                        if (gameHandler.player.cards == 0)
                        {
                            gameHandler.player.Damage(40, DamageTypeDeprecated.Fire, gameHandler.opponent);
                            Debug.Log("Player has no cards! Dealing damage automatically.");
                            Discard(selectedCard, true);
                            gameHandler.opponent.diamondsBeforeExhaustion--;
                            StartCoroutine(OpponentCardLogic());
        ...
        foreach (Transform child in gameHandler.opponentArea.transform)
        {
            ...
            if (selectedCardComponent.cardSuit == CardSuit.SPADE && gameHandler.opponent.spadesBeforeExhaustion != 0)
            {
                ...
                else
                {
                    ...
                    {
                        if (kid.gameObject.GetComponent<Card>().cardSuit == CardSuit.HEART && gameHandler.opponent.currentHP < 0.75f * gameHandler.opponent.maxHP)
						{
    ...
    public IEnumerator AttackCalc(GameObject attackingCard)
	{
        ...
        if (attackingCardComponent.cardValue > opponentCardComponent.cardValue)
        {
            ...
            {
                case "The Wraith King":
                    if (attackingCardComponent.cardSuit == CardSuit.CLUB)
                    {
        ...
        switch (cardComponent.cardValue)
        {
            ...
            case 4:
                ...
                if (Random.Range(0f, 1f) > chance && gameHandler.opponent.currentHP > 20 || gameHandler.opponent.cards == 0)
                {
                    ...
                    gameHandler.opponent.Damage(20, DamageTypeDeprecated.Unblockable, gameHandler.player);
                    gameHandler.player.diamondsBeforeExhaustion--;
                    break;
                }
            ...
            case 10:
                ...
                break;
            case 11:
                gameHandler.opponent.Damage(20, DamageTypeDeprecated.Fire, gameHandler.player);
                Discard(card);
                break;
            case 12:
                ...
                if (Random.Range(0f, 1f) > chance && gameHandler.opponent.currentHP > 40 || gameHandler.opponent.cards == 0)
                {
                    ...
                    gameHandler.opponent.Damage(40, DamageTypeDeprecated.Fire, gameHandler.player);
                    gameHandler.player.diamondsBeforeExhaustion--;
                    break;
                }
